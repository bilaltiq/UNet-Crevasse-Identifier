...
# ────────────init model ──────────────
model = tfUNetplus.unet_plus_plus_model(
    input_shape=(512,512,1), num_classes=1, deep_supervision=False)
...
# ───── save model & curves ─────
model.save('jupyter-kernels/UNet_Scripts/unetpp_crevasse.keras')
...
model_save_dir = 'jupyter-kernels/UNet_Scripts'
model_name = 'unetpp_test_model.keras'
pp = os.path.join(model_save_dir, model_name)
model.save(pp)


save_dir  = 'jupyter-kernels/UNet_Scripts'
os.makedirs(save_dir, exist_ok=True)
save_path = os.path.join(save_dir, 'Crevasse_unetpptraining_history.png')
